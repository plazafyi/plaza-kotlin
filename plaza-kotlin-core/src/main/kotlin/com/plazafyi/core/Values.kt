...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw PlazaInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw PlazaInvalidDataException("`$name` is not set")
            is JsonNull -> throw PlazaInvalidDataException("`$name` is null")
            else -> throw PlazaInvalidDataException("`$name` is invalid, received $this")
        }
